  ...
  return (
    <div className="h-auto flex flex-col justify-center items-center bg-cover bg-center p-4" style={{ backgroundImage: 'url(/background/pomodoro.jpeg)', fontFamily: 'Poppins, sans-serif' }}>
      <div className="flex space-x-1 mb-8">
        <button onClick={setPomodoro} className={buttonClass('pomodoro')}>pomodoro</button>
        <button onClick={setShortBreak} className={buttonClass('short break')}>short break</button>
        <button onClick={setLongBreak} className={buttonClass('long break')}>long break</button>
      </div>
      <div className="text-white text-5xl font-bold mb-8">{formatTime()}</div>
      <div className="flex space-x-4">
        <button onClick={toggleTimer} className="bg-white text-black hover:bg-gray-200 font-bold text-sm py-1.5 px-6 rounded-full shadow-md focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50">
          {isActive ? 'Pause' : 'Start'}
        </button>
        <button onClick={resetTimer}>
          <span className="inline-block transform rotate-0"><RestartIcon className="text-white h-6 w-6" /></span>
        </button>
      </div>
    </div>
  );
